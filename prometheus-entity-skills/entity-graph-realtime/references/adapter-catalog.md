...
Source: **`src/adapters/types.ts`**, **`src/adapters/realtime-adapters.ts`**, **`src/adapters/flint.ts`**, **`src/adapters/electricsql.ts`**, **`src/adapters/realtime-manager.ts`**. All factories are re-exported from the **package root** (`src/index.ts`).
...
## `createFlintAdapter(options: CreateFlintAdapterOptions)`

**Use when:** a caller-owned Flint entity-management client supplies
`watchEntities(query)` and `mutateEntity(record)`.

| Option | Role |
|--------|------|
| **`client`** | Structural `FlintClientLike`; the package bundles no Flint SDK |
| **`channelId`**, **`consumerId`** | Required subscription and checkpoint identity |
| **`entityType?`** | Optional entity filter applied to query and received events |
| **`checkpoints?`** | Optional bigint offset store; resume starts at the last offset plus one |
| **`resolveOp?`** | Event-to-`ChangeOperation` mapping; defaults to `upsert` |

Use `publishFlintMutation(client, record)` for the matching outbound
`mutateEntity` call. The consumer owns authenticated client construction and
lifecycle. Keep service-role credentials on the server; client identity and
tenant selection never replace Gate authorization.

Before claiming compatibility, load
**`../../_shared/references/flint-portable-contracts.md`** and run
`pnpm run verify:flint-contracts`. Current RSA JWKs have standard `n`/`e`;
strict EC consumption remains unsupported because `crv`/`x`/`y` are absent.
Forge provisioning is an external operator flow, not an adapter generated by
this skill.
