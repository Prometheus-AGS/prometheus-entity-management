...
import { registerEntityTransport, __resetEntityTransports } from "../transport/registry";
import { TerminalError, TransientError } from "../errors";
import type { EntityTransport, ListResult } from "../transport/types";
...
async function simulateFetch<T extends object>(
  ...
  const { getEntityTransport } = await import("../transport/registry");
  const { toEntityError } = await import("../errors");
  ...
  try {
    ...
    const { toEntityError: toErr, TransientError: TE } = await import("../errors");
    ...
    const gs = useGraphStore.getState();
    const te = (await import("../errors")).toEntityError(err);
