...
All twelve npm packages are public at stable `3.2.0` on the `latest` tag.
...
| `@prometheus-ags/prometheus-entity-management@3.2.0` | React 19 bindings: hooks, CRUD, UI components, GraphQL, Prisma helpers | `pnpm run verify:vite-react19` |
...
| `@prometheus-ags/a2ui-react@3.2.0` | A2UI v1.0-RC compatibility over the official v0.9.1 renderer + AG-UI 0.0.59 activity/chat entry points | `pnpm run verify:a2ui-bridge` |
...
| Package | Use it for | Evidence gate |
| ------- | ---------- | ------------- |
| `entity_graph_flutter@3.1.0` | Canonical Dart graph, Riverpod providers, optional DevTools controller, and official Flutter DevTools companion | `pnpm run verify:devtools-flutter-controller` + official extension validation |
