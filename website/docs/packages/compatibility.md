...
| --- | --- | --- |
| Node.js | `^22.14`, `^24`, or `>=26` | all twelve npm packages public at `3.0.5` |
| React | React 19 binding, vanilla core underneath | React and core both `3.0.5` on `latest` |
| Svelte | Svelte 5 peer binding | public at `3.0.5` on `latest` |
| Solid | Solid 1.x peer binding | public at `3.0.5` on `latest` |
| Flutter | Flutter 3.44.8 / Riverpod 3 | pub.dev `entity_graph_flutter@3.0.1`; A2UI 1.0-RC compatibility via GenUI 0.10.2 |
| Tauri | Tauri 2 JavaScript/Rust plugin | public at `3.0.5`; native signing excluded |
...
now matches registry state: every npm package has moved `latest` to `3.0.5`;
Flutter is a public stable `3.0.1` on pub.dev. React and A2UI React are public
at `3.0.5`, with AG-UI 0.0.59 transport compatibility.
