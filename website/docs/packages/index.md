...
| --- | --- | --- |
| `@prometheus-ags/prometheus-entity-management` (React) | `pnpm add @prometheus-ags/prometheus-entity-management` | `3.0.5` |
| `@prometheus-ags/entity-graph-core` | `pnpm add @prometheus-ags/entity-graph-core` | `3.0.5` |
| Other ten `@prometheus-ags/*` packages | `pnpm add <package>` | `3.0.5` |
| `entity_graph_flutter` | pub.dev `3.0.1` | `3.0.1` |

Installs resolve stable `3.0.5` from `latest` by default. The withdrawn
`3.0.0` manifests are deprecated (unresolved `workspace:` protocol), and
stale-artifact `3.0.4` is also deprecated. `@next` now points at stable `3.0.5`
