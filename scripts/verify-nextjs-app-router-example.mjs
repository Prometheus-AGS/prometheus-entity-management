...
run("nextjs-typecheck", "pnpm", [
  "--filter",
  "prometheus-entity-management-nextjs",
  "typecheck",
]);
run("ssr-isolation-unit", "pnpm", [
  "--filter",
  "prometheus-entity-management-nextjs",
  "run",
  "test:ssr-isolation",
...
run("nextjs-production-build", "pnpm", [
  "--filter",
  "prometheus-entity-management-nextjs",
  "build",
]);
run("nextjs-production-browser", "pnpm", [
  "exec",
  "playwright",
  "test",
  "--config",
...
if (!showcase) throw new Error("examples/coverage.json has no nextjs showcase");
...
const report = {
  ...
  versions: {
    ...
    react: readJson("examples/nextjs-app/package.json").dependencies.react,
    next: readJson("examples/nextjs-app/package.json").dependencies.next,
    playwright: readJson("package.json").devDependencies["@playwright/test"],
    typescript: readJson("examples/nextjs-app/package.json").devDependencies.typescript,
    packageCandidate: packageVersion("packages/entity-graph-react/package.json"),
  },
  commands,
  unit: {
    status: "pass",
    testFiles: ["examples/nextjs-app/src/lib/server/request-isolation.test.ts"],
    concurrentTenantPayloadsDisjoint: true,
    perRequestGraphsSerializable: true,
    globalStoreUntouchedByServerPath: true,
    hydratedListsSatisfyStaleTimePredicate: true,
  },
  production: {
    typecheck: "pass",
    build: "pass",
    app: "examples/nextjs-app",
    dynamicSsrRoute: "/release-showcase",
  },
  browser: {
    status: "pass",
    ...
    hydrationMismatchErrors: 0,
    duplicateHydratedListFetches: 0,
  },
...
console.log(`Next.js App Router verification passed: ${relative(root, reportPath)}`);
