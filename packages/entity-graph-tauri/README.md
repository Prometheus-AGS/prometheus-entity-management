...
Desktop proof is not mobile proof. Android and iOS require the simulator/device procedure in [`release/tauri-mobile-device-lane.md`](../../release/tauri-mobile-device-lane.md); the npm package is published at stable `3.0.3`, but mobile runtime certification remains outstanding until those artifacts are captured.
