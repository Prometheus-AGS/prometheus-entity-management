...
## Iteration 4 — 2026-08-29T18:54:44Z

### Delta

- Round 3 found fetch/sync changes updating raw metadata keys while record
  projections read JSON-encoded identities.
- Naive first-colon parsing could create a phantom identity when actual entity
  types or IDs contain colons.
- The staged/unstaged packet contained duplicate post-images and KBD's generated
  progress projection still selected already-cancelled legacy tasks.

### Actions taken

- Resolved legacy metadata keys against actual graph entity/patch buckets,
  falling back only when no concrete identity exists.
- Advanced fetch/sync entity revisions under the same collision-free identity
  used by records, views, previews, and canonical/patch changes.
- Expanded the assembled gate to prove both colliding identities advance and no
  phantom record appears; all 16 scenarios pass.
- Confirmed through signed control-plane 409 responses that tasks 6–8 are
  already cancelled; the stale `next_task_pending` text is a projection defect,
  not pending implementation.
- Restage the complete final tree before packet generation so each file has one
  post-image and every new source/fixture is present.
...
- Convergence: terminate.
- Reason: revision identity semantics now match the public contract and the
  full staged change can be reviewed coherently.
